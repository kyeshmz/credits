...
An open-source wiki of startup credit programs — cloud credits, AI/API credits, and SaaS discounts for early-stage startups.

Every entry links to its source and shows when it was last checked. Values and eligibility change often; verify on the provider's page before relying on any entry.

- Site: `apps/credits-wiki` (Astro, static output, models.dev-derived style)
- Data: `packages/credits-data/src/credits.json` (typed + validated JSON)
- Live site `site` is set in `apps/credits-wiki/astro.config.mjs` (`https://credits.kyeshimizu.com`)

## Repo layout

```
apps/credits-wiki/          Astro site
